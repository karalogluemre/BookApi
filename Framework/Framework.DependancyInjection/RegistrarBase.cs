...
namespace Framework.DependencyInjection
{
    public abstract class RegistrarBase<TRegister> : IRegistrar
    {
        ...
        {
            services.AddDbContext<IDbContext, LibraryDbContext>(options =>
            {
        ...
        private void RegisterCommandFacade(IServiceCollection services)
        {
            ...
            foreach (var repository in repositories)
            {
                var baseInterfaces = repository.GetInterfaces().Where(a => a.IsGenericType == false && a.Namespace.StartsWith(nameof(Books)));
                foreach (var baseInterface in baseInterfaces)
                    services.AddScoped(baseInterface, repository);
            }
        }
